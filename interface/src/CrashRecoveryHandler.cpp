...
CrashRecoveryHandler::Action CrashRecoveryHandler::promptUserForAction(bool showCrashMessage) {
    ...
    QLabel* label;
    if (showCrashMessage) {
        crashDialog.setWindowTitle("Settings startup manager");
        label = new QLabel("Would you like to reset your local settings? ");
    } else {
        crashDialog.setWindowTitle("Settings startup manager");
        label = new QLabel("Would you like to reset your local settings?");
    }
    ...
    QRadioButton* option1 = new QRadioButton("Reset all my settings.  Avatar and content may be lost.");
