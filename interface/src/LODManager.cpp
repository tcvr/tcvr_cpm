...
void LODManager::autoAdjustLOD(float realTimeDelta) {
    ...
    // CPM Just force automatic LOD Adjust to false.  I think it's a terrible experience.
    _automaticLODAdjust = false;  // CPM I added this here to force to false regardless of other settings.
    ...
    if (!_automaticLODAdjust || realTimeDelta <= 0.0f || _nowRenderTime <= 0.0f || _smoothRenderTime <= 0.0f) { // CPM This is where it earlies out and avoids automatic adjust
        return;
    }
...
void LODManager::setAutomaticLODAdjust(bool value) {
   // qCDebug(interfaceapp) << "CPM - LOD Manager SET AUTOMATIC LOD ADJUST IS " << _automaticLODAdjust;
...
QString LODManager::getLODFeedbackText() {
    //qCDebug(interfaceapp) << "CPM - LOD Manager Get LOD Feedback Text settings";
...
void LODManager::loadSettings() {
    //qCDebug(interfaceapp) << "CPM - LOD Manager load settings";
...
void LODManager::saveSettings() {
   // qCDebug(interfaceapp) << "CPM - LOD Manager save settings";
...
// CPM Why do we even call it "World Detail Quality". It seems inaccurate to call the culling of content "Quality".
...
void LODManager::setLODQualityLevel(float quality) {
   // qCDebug(interfaceapp) << "CPM - SET LOD QUALITY TO " << quality;
    _lodQualityLevel = quality;
}

float LODManager::getLODQualityLevel() const {
   // qCDebug(interfaceapp) << "CPM - GET LOD QUALITY IS " << _lodQualityLevel;
