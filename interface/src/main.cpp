...
int main(int argc, const char* argv[]) {
    ...
    Setting::init();  // CPM look into setting class in SettingInterface.cpp

    
    qInfo() << "CPM - HERE'S MAIN 3!";
    ...
    qInfo() << "CPM - HERE'S MAIN 4!";
    const QString& applicationName = getInterfaceSharedMemoryName();
    bool instanceMightBeRunning = true;

    qCDebug(interfaceapp, "CPM main 0aaaaaaaaaaaaa.");
#ifdef Q_OS_WIN
    ...
    if (instanceMightBeRunning) {
        ...
        qInfo() << "CPM - HERE'S MAIN 6!";
#ifdef Q_OS_WIN
        return EXIT_SUCCESS;
#endif
    }

    qCDebug(interfaceapp, "CPM main 0aaaaaa.");
    ...
    qCDebug(interfaceapp, "CPM main 0aaa.");
#ifdef Q_OS_WIN
    ...
    qCDebug(interfaceapp, "CPM main 0aa.");
    int exitCode; // CPM What's this?
    {
        qCDebug(interfaceapp) << "CPM Exit code called 1.";
        ...
        qCDebug(interfaceapp) << "CPM Exit code called21.";
        ...
        qCDebug(interfaceapp) << "CPM main - 311.";
        ...
        qCDebug(interfaceapp) << "CPM main - 316.";
#ifdef Q_OS_ANDROID
        ...
        // CPM****************************************************************************************************************************
        Application app(argcExtended, const_cast<char**>(argvExtended.data()), startupTime, runningMarkerExisted); // CPM APP STARTS HERE?
        qCDebug(interfaceapp) << "CPM main - 326 - App was started on ~325."; // **********************************************************
        PROFILE_SYNC_END(startup, "app full ctor", "");        
        
        qCDebug(interfaceapp) << "CPM main - 329.";
        QTimer exitTimer;
        if (traceDuration > 0.0f) {
            qCDebug(interfaceapp) << "CPM Exit code called 4 trace duration?.";
            exitTimer.setSingleShot(true);
            QObject::connect(&exitTimer, &QTimer::timeout, &app, &Application::quit);
            exitTimer.start(int(1000 * traceDuration));
        }

        qCDebug(interfaceapp, "CPM main 0a       **************************************");
#if 0
        ...
        qCDebug(interfaceapp, "CPM main 0.");
        // Setup local server
        ...
        qCDebug(interfaceapp) << "CPM main shutdownplugins called 1.";
    ...
    qCDebug(interfaceapp) << "CPM Exit code weird hack area";
    _exit(exitCode);
#endif
    return exitCode;
}
