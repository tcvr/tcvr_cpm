...
bool MyAvatar::findAvatarEntity_LOD(const QString& modelURL,
                                    const QString& modelURL_LOD1,
                                    const QString& modelURL_LOD2,
                                    const QString& modelURL_LOD3,
                                    const QString& jointName,
                                    QUuid& entityID) {
    QList<QUuid> avatarEntityIDs;
    _avatarEntitiesLock.withReadLock([&] { avatarEntityIDs = _packedAvatarEntityData.keys(); });
    for (const auto& entityID : avatarEntityIDs) {
        auto props = DependencyManager::get<EntityScriptingInterface>()->getEntityProperties(entityID);
        if (props.getModelURL() == modelURL &&
            (jointName.isEmpty() || props.getParentJointIndex() == getJointIndex(jointName))) {
        ...
        if (props.getModelURL_LOD1() == modelURL_LOD1 &&  // CPM
            (jointName.isEmpty() || props.getParentJointIndex() == getJointIndex(jointName))) {
            return true;
        }
        if (props.getModelURL_LOD2() == modelURL_LOD2 &&  // CPM
            (jointName.isEmpty() || props.getParentJointIndex() == getJointIndex(jointName))) {
            return true;
        }
        if (props.getModelURL_LOD3() == modelURL_LOD3 &&  // CPM
            (jointName.isEmpty() || props.getParentJointIndex() == getJointIndex(jointName))) {
            return true;
        }
    }
...
AttachmentData MyAvatar::entityPropertiesToAttachmentData(const EntityItemProperties& properties) const {
    ...
    data.modelURL_LOD1 = properties.getModelURL_LOD1();  // CPM
    data.modelURL_LOD2 = properties.getModelURL_LOD2();  // CPM
    data.modelURL_LOD3 = properties.getModelURL_LOD3();  // CPM
