...
//  interface/src
//
//  Modified by Caitlyn Meeks, Tivoli Cloud
//  May 1 2019
//
...
// CPM Added path to a stylesheet
//static const QUrl STYLESHEET_QML = PathUtils::qmlUrl("stylesheet.qml");

// CPM Mic Bar on HMD QML I believe
...
#endif
const std::vector<std::pair<QString, Application::AcceptURLMethod>>
...
public:
    LambdaEvent(const std::function<void()>& fun) : QEvent(static_cast<QEvent::Type>(ApplicationEvent::Lambda)), _fun(fun) {}
    LambdaEvent(std::function<void()>&& fun) : QEvent(static_cast<QEvent::Type>(ApplicationEvent::Lambda)), _fun(fun) {}
...
// CPM setup essentials
bool setupEssentials(int& argc, char** argv, bool runningMarkerExisted) {
    qCDebug(interfaceapp) << "CPM - Setup Essentials";
...
#else
    DependencyManager::set<AccountManager>(std::bind(&Application::getUserAgent, qApp));  // cpm interesting
#endif
...
Application::Application(int& argc, char** argv, QElapsedTimer& startupTimer, bool runningMarkerExisted) :
    QApplication(argc, argv), _window(new MainWindow(desktop())), _sessionRunTimer(startupTimer),  // Our ap
#ifndef Q_OS_ANDROID
    ...
    _firstRun(Settings::firstRun, true),  // CPM is this where settings are initialized??
    ...
    qInfo() << "CPM - Plugin loader - Application ";

    qInfo() << "CPM - Application constructor";
    ...
    // CPM added: NVM NVM
    // const QUrl DARK_STYLESHEET = PathUtils::qmlUrl("DarkStylesheet.qml"); // CPM  AINT GONNA WORK VM
    // QQmlApplicationEngine engine; // CPM
    // engine.load(DARK_STYLESHEET); // CPM
    ...
    _window->setWindowTitle(appTitleString);
    ...
    DependencyManager::get<WalletScriptingInterface>()->setLimitedCommerce(
    ...
    connect(&domainHandler, &DomainHandler::disconnectedFromDomain, DependencyManager::get<ScriptCache>().data(),
    ...
    connect(&locationUpdateTimer, &QTimer::timeout, DependencyManager::get<AddressManager>().data(),
    ...
    connect(myAvatar.get(), &MyAvatar::positionGoneTo, DependencyManager::get<AddressManager>().data(),
            &AddressManager::storeCurrentAddress);
    ...
    connect(userInputMapper.data(), &UserInputMapper::actionEvent, [this](int action, float state) {
        ...
        auto audioScriptingInterface =
            reinterpret_cast<scripting::Audio*>(DependencyManager::get<AudioScriptingInterface>().data());
        {
...
// FIXME spacemouse code still needs cleanup
    ...
    // Monitor model assets (e.g., from Clara.io) added to the world that may need resizing. CPM Is this weird?
    ...
    DependencyManager::get<PickManager>()->setShouldPickHUDOperator(
    ...
    EntityItem::setBillboardRotationOperator([this](const glm::vec3& position, const glm::quat& rotation,
    ...
    DependencyManager::get<UsersScriptingInterface>()->setKickConfirmationOperator(
        [this](const QUuid& nodeID) { userKickConfirmation(nodeID); });

    render::entities::WebEntityRenderer::setAcquireWebSurfaceOperator([this](const QString& url, bool htmlContent,
    ...
    render::entities::WebEntityRenderer::setReleaseWebSurfaceOperator(
        [this](QSharedPointer<OffscreenQmlSurface>& webSurface, bool& cachedWebSurface,
...
QString Application::getUserAgent() {
    qInfo() << "CPM -Get User Agent as qInfo ";
    qCDebug(interfaceapp) << "CPM - Get User Agent";
...
void Application::onAboutToQuit() {
    // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITON ABOUT TO QUIT 1";
    // quickly save AvatarEntityData before the EntityTree is dismantled
    getMyAvatar()->saveAvatarEntityDataToSettings();

    // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITON ABOUT TO QUIT 2";
    ...
    // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITON ABOUT TO QUIT 3";
    ...
    // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITON ABOUT TO QUIT 4";
    ...
    // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITON ABOUT TO QUIT 5";
    ...
    // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITON ABOUT TO QUIT 6";
    ...
    // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITON ABOUT TO QUIT 7";
    _aboutToQuit = true;

    // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITON ABOUT TO QUIT 8";
    cleanupBeforeQuit();
}

void Application::cleanupBeforeQuit() {
    // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITCLEANUP BEFORE QUIT 1";
    // add a logline indicating if QTWEBENGINE_REMOTE_DEBUGGING is set or not
    QString webengineRemoteDebugging = QProcessEnvironment::systemEnvironment().value("QTWEBENGINE_REMOTE_DEBUGGING", "false");
    // qCDebug(interfaceapp) << "QTWEBENGINE_REMOTE_DEBUGGING =" << webengineRemoteDebugging;

    // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITCLEANUP BEFORE QUIT 2";
    ...
    // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITCLEANUP BEFORE QUIT 3";
    ...
    // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITCLEANUP BEFORE QUIT 4";
    AnimDebugDraw::getInstance().shutdown();

    // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITCLEANUP BEFORE QUIT 5";
    ...
    // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITCLEANUP BEFORE QUIT 6";
    {
        auto nodeList = DependencyManager::get<NodeList>();
    ...
    // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITCLEANUP BEFORE QUIT 7";
    getEntities()->shutdown();  // tell the entities system we're shutting down, so it will stop running scripts

    // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITCLEANUP BEFORE QUIT 8";
    // Clear any queued processing (I/O, FBX/OBJ/Texture parsing)
    QThreadPool::globalInstance()->clear();

    // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITCLEANUP BEFORE QUIT 9";
    ...
    // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITCLEANUP BEFORE QUIT 10";
    // CPM Hack
    // CPM To make a much faster shutdown I am commenting out shutdownScripting on line below:
    ...
    {
        // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITCLEANUP BEFORE QUIT 10a";
        DependencyManager::destroy<StandAloneJSConsole>();
        // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITCLEANUP BEFORE QUIT 10b";
        EntityTreePointer tree = getEntities()->getTree();
        // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITCLEANUP BEFORE QUIT 10c";
        tree->setSimulation(nullptr);
        // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITCLEANUP BEFORE QUIT 10d";
        DependencyManager::destroy<EntityTreeRenderer>();
        // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITCLEANUP BEFORE QUIT 10e";
    }

    // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITCLEANUP BEFORE QUIT 11";
    ...
    // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITCLEANUP BEFORE QUIT 12";
    _displayPlugin.reset();
    PluginManager::getInstance()->shutdown();

    // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITCLEANUP BEFORE QUIT 13";
    // Cleanup all overlays after the scripts, as scripts might add more
    _overlays.cleanupAllOverlays();

    // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITCLEANUP BEFORE QUIT 14";
    ...
    // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITCLEANUP BEFORE QUIT 15";
    ...
    // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITCLEANUP BEFORE QUIT 16";
    DependencyManager::destroy<ContextOverlayInterface>();  // Must be destroyed before TabletScriptingInterface

    // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITCLEANUP BEFORE QUIT 17";
    // stop QML
    DependencyManager::destroy<QmlCommerce>();

    // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITCLEANUP BEFORE QUIT 18";
    DependencyManager::destroy<TabletScriptingInterface>();

    // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITCLEANUP BEFORE QUIT 19";
    DependencyManager::destroy<ToolbarScriptingInterface>();

    // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITCLEANUP BEFORE QUIT 20";
    DependencyManager::destroy<OffscreenUi>();

    // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITCLEANUP BEFORE QUIT 21";
    DependencyManager::destroy<OffscreenQmlSurfaceCache>();

    _snapshotSoundInjector = nullptr;

    // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITCLEANUP BEFORE QUIT 22";
    // destroy Audio so it and its threads have a chance to go down safely
    // this must happen after QML, as there are unexplained audio crashes originating in qtwebengine
    QMetaObject::invokeMethod(DependencyManager::get<AudioClient>().data(), "stop");
    DependencyManager::destroy<AudioClient>();

    // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITCLEANUP BEFORE QUIT 23";

    DependencyManager::destroy<AudioInjectorManager>();
    DependencyManager::destroy<AudioScriptingInterface>();

    // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITCLEANUP BEFORE QUIT 24";
    ...
    // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITCLEANUP BEFORE QUIT 25";
    DependencyManager::destroy<KeyboardScriptingInterface>();
    DependencyManager::destroy<Keyboard>();

    // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITCLEANUP BEFORE QUIT 26";
    DependencyManager::destroy<AvatarPackager>();

    // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITCLEANUP BEFORE QUIT 27";
    // qCDebug(interfaceapp) << "Application::cleanupBeforeQuit() complete";
}

// CPM Deconstructor
Application::~Application() {
    // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITQuit";
    // remove avatars from physics engine
    auto avatarManager = DependencyManager::get<AvatarManager>();
    avatarManager->clearOtherAvatars();
    // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITQuit 2";
    ...
    // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITQuit 3";
    ...
    // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITQuit 4";
    ...
    // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITQuit 5";
    _physicsEngine->setCharacterController(nullptr);

    // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITQuit6";
    ...
    // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITQuit 7";
    _gameWorkload.shutdown();

    // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITQuit 8";
    DependencyManager::destroy<Preferences>();

    // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITQuit 9";
    _entityClipboard->eraseAllOctreeElements();
    _entityClipboard.reset();

    // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITQuit 10";
    _octreeProcessor.terminate();

    // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITQuit 11";
    _entityEditSender.terminate();

    // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITQuit 12";
    ...
    // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITQuit13";
    DependencyManager::destroy<PluginManager>();

    // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITQuit 14";
    DependencyManager::destroy<CompositorHelper>();  // must be destroyed before the FramebufferCache

    // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITQuit 15";
    DependencyManager::destroy<SoundCacheScriptingInterface>();

    // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITQuit 16";
    DependencyManager::destroy<AvatarManager>();

    // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITQuit 17";
    DependencyManager::destroy<AnimationCacheScriptingInterface>();
    DependencyManager::destroy<AnimationCache>();

    // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITQuit 18";
    DependencyManager::destroy<FramebufferCache>();
    DependencyManager::destroy<TextureCacheScriptingInterface>();
    DependencyManager::destroy<TextureCache>();

    // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITQuit 19";
    DependencyManager::destroy<ModelCacheScriptingInterface>();
    DependencyManager::destroy<ModelCache>();
    DependencyManager::destroy<ModelFormatRegistry>();

    // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITQuit 20";
    DependencyManager::destroy<ScriptCache>();

    // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITQuit 21";
    DependencyManager::destroy<SoundCacheScriptingInterface>();
    DependencyManager::destroy<SoundCache>();

    // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITQuit 22";
    DependencyManager::destroy<OctreeStatsProvider>();

    // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITQuit23 ";
    DependencyManager::destroy<GeometryCache>();

    // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITQuit 24";
    DependencyManager::get<ResourceManager>()->cleanup();

    // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITQuit 25";
    ...
    // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITQuit 26";

    // The window takes ownership of the menu, so this has the side effect of destroying it.
    _window->setMenuBar(nullptr);

    // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITQuit 27";
    _window->deleteLater();

    // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITQuit 28";
    ...
    // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITQuit 29";
    // quit the thread used by the closure event sender
    closeEventSender->thread()->quit();

    // qCDebug(interfaceapp) << "CPM ON ABOUT TO QUITQuit 30";
...
void Application::showLoginScreen() {
    ...
    if (!accountManager->isLoggedIn()) {
        ...
        if (!isHMDMode()) {
            auto toolbar =
                DependencyManager::get<ToolbarScriptingInterface>()->getToolbar("com.highfidelity.interface.toolbar.system"); // CPM IS THIS FOR REAL? Is this where the toolbar is loaded?!?
            toolbar->writeProperty("visible", false);
        }
        */
        _loginDialogPoppedUp = true;
        dialogsManager->showLoginDialog(); // CPM
        emit loginDialogFocusEnabled();
        QJsonObject loginData = {};
        loginData["action"] = "login dialog popped up";
        UserActivityLogger::getInstance().logAction("encourageLoginDialog", loginData);
        _window->setWindowTitle("Tivoli Cloud VR"); // CPM
    } else {
        resumeAfterLoginDialogActionTaken();
    }
...
void Application::setupQmlSurface(QQmlContext* surfaceContext, bool setAdditionalContextProperties) {
    ...
    if (setAdditionalContextProperties) {
        ...
        surfaceContext->setContextProperty("WalletScriptingInterface",
                                           DependencyManager::get<WalletScriptingInterface>().data());
        surfaceContext->setContextProperty("ResourceRequestObserver", DependencyManager::get<ResourceRequestObserver>().data());
    }
}
...
void Application::showHelp() {
    ...
    TabletProxy* tablet =
        dynamic_cast<TabletProxy*>(DependencyManager::get<TabletScriptingInterface>()->getTablet(SYSTEM_TABLET));
...
void Application::handleSandboxStatus(QNetworkReply* reply) {
    ...
    static const std::map<HandControllerType, int> MIN_CONTENT_VERSION = { { Vive, 1 }, { Oculus, 27 } };
                ...
                if (!isShifted && !isMeta && !isOption && !event->isAutoRepeat()) {
                    ...
                    if (notificationSounds.get() && notificationSoundSnapshot.get()) {
                        if (_snapshotSoundInjector) {
                            DependencyManager::get<AudioInjectorManager>()->setOptionsAndRestart(_snapshotSoundInjector,
                                                                                                 options);
                        } else {
                            _snapshotSoundInjector =
                                DependencyManager::get<AudioInjectorManager>()->playSound(_snapshotSound, options);
                        }
                    }
                    takeSnapshot(true);
                }
...
void Application::idle() {
    ...
    PROFILE_COUNTER_IF_CHANGED(app, "currentProcessing", int,
                               DependencyManager::get<StatTracker>()->getStat("Processing").toInt());
    PROFILE_COUNTER_IF_CHANGED(app, "pendingProcessing", int,
                               DependencyManager::get<StatTracker>()->getStat("PendingProcessing").toInt());
    auto renderConfig = _graphicsEngine.getRenderEngine()->getConfiguration();
...
bool Application::exportEntities(const QString& filename, const QVector<QUuid>& entityIDs, const glm::vec3* givenOffset) {
...
// CPM Initialize the application
...
void Application::pauseUntilLoginDetermined() {
    ...
    // _developerMenuVisible = menu->getMenu("Developer")->isVisible(); // CPM COMMENTED OUT
    _developerMenuVisible = true;  // CPM Forced developer menu on by default -- also in Application.h
...
void Application::resumeAfterLoginDialogActionTaken() {
    ...
    if (!isHMDMode() && getDesktopTabletBecomesToolbarSetting()) {
        auto toolbar =
            DependencyManager::get<ToolbarScriptingInterface>()->getToolbar("com.highfidelity.interface.toolbar.system");
    ...
    menu->getMenu("Developer")->setVisible(_developerMenuVisible);  // CPM - forced to always true in Application.h
...
void Application::update(float deltaTime) {
    ...
    auto myAvatar = getMyAvatar();
    {
        ...
        static const std::vector<controller::Action> avatarControllerActions = { controller::Action::LEFT_HAND,
    ...
    // send packet containing downstream audio stats to the AudioMixer
    {
        ...
        if (sinceLastNack > TOO_LONG_SINCE_LAST_SEND_DOWNSTREAM_AUDIO_STATS && !isInterstitialMode()) {
            ...
            QMetaObject::invokeMethod(DependencyManager::get<AudioClient>().data(), "sendDownstreamAudioStatsPacket",
                                      Qt::QueuedConnection);
        }
    }
...
void Application::updateWindowTitle() const {
    ...
    // CPM  - Added appTitleString
    QString title = QString() + appTitleString + " - " + (!username.isEmpty() ? username + " @ " : QString()) +
...
void Application::registerScriptEngineWithApplicationServices(ScriptEnginePointer scriptEngine) {
    ...
    qScriptRegisterMetaType(scriptEngine.data(), wrapperToScriptValue<ToolbarButtonProxy>,
                            wrapperFromScriptValue<ToolbarButtonProxy>);
    ...
    qScriptRegisterMetaType(scriptEngine.data(), wrapperToScriptValue<TabletButtonProxy>,
                            wrapperFromScriptValue<TabletButtonProxy>);
    ...
    scriptEngine->registerGlobalObject("UserActivityLogger",
                                       DependencyManager::get<UserActivityLoggerScriptingInterface>().data());
...
bool Application::acceptURL(const QString& urlString, bool defaultUpload) {
    ...
    if (url.scheme() == URL_SCHEME_HIFI) {
        ...
        QMetaObject::invokeMethod(DependencyManager::get<AddressManager>().data(), "handleLookupString", Qt::AutoConnection,
                                  Q_ARG(const QString&, urlString));
        return true;
    }
...
bool Application::askToSetAvatarUrl(const QString& url) {
    ...
    auto setAvatar = [=](QString url, QString modelName) {
        ModalDialogListener* dlg =
            OffscreenUi::asyncQuestion("Set Avatar", "Would you like to use '" + modelName + "' for your avatar?",
    ...
    if (!modelLicense.isEmpty()) {
        ...
        ModalDialogListener* dlg =
            OffscreenUi::asyncQuestion("Avatar Usage License", modelLicense + "\nDo you agree to these terms?",
    ...
    connect(reply, &QNetworkReply::finished, [this, reply, url, requestNumber]() {
        ...
        if (networkError == QNetworkReply::NoError) {
            ...
            if (jsonError.error == QJsonParseError::NoError) {
                ...
                ModalDialogListener* dlg =
                    OffscreenUi::asyncQuestion(avatarAttachmentConfirmationTitle, avatarAttachmentConfirmationMessage,
...
bool Application::askToReplaceDomainContent(const QString& url) {
    ...
    if (DependencyManager::get<NodeList>()->getThisNodeCanReplaceContent()) {
        ...
        if (originURL.host().endsWith(MARKETPLACE_CDN_HOSTNAME)) {
            ...
            static const QString infoText =
                simpleWordWrap(
                    "Your domain's content will be replaced with a new content set. "
                    "If you want to save what you have now, create a backup before proceeding. For more information about "
                    "backing up "
                    "and restoring content, visit the documentation page at: ",
                    MAX_CHARACTERS_PER_LINE) +
                "\nhttps://docs.highfidelity.com/create-and-explore/start-working-in-your-sandbox/restoring-sandbox-content";

            ModalDialogListener* dig =
                OffscreenUi::asyncQuestion("Are you sure you want to replace this domain's content set?", infoText,
            ...
            QObject::connect(dig, &ModalDialogListener::response, this, [=](QVariant answer) {
                ...
                QJsonObject messageProperties = { { "status", details }, { "content_set_url", url } };
            ...
            QJsonObject messageProperties = { { "status", methodDetails }, { "content_set_url", url } };
            UserActivityLogger::getInstance().logAction("replace_domain_content", messageProperties);
        }
    } else {
        methodDetails = "UserDoesNotHavePermissionToReplaceContent";
        static const QString warningMessage = simpleWordWrap(
            "The domain owner must enable 'Replace Content' "
            "permissions for you in this domain's server settings before you can continue.",
            MAX_CHARACTERS_PER_LINE);
        OffscreenUi::asyncWarning("You do not have permissions to replace domain content", warningMessage, QMessageBox::Ok,
                                  QMessageBox::Ok);
...
void Application::addAssetToWorldFromURL(QString url) {
    ...
    auto request = DependencyManager::get<ResourceManager>()->createResourceRequest(nullptr, QUrl(url), true, -1,
                                                                                    "Application::addAssetToWorldFromURL");
...
void Application::addAssetToWorld(QString path, QString zipFile, bool isZip, bool isBlocks) {
    return;  // CPM Not supporting the drag drop pipeline at the moment
...
void Application::addAssetToWorldWithNewMapping(QString filePath, QString mapping, int copy, bool isZip, bool isBlocks) {
    ...
    QObject::connect(request, &GetMappingRequest::finished, this, [=](GetMappingRequest* request) mutable {
        ...
        if (result == GetMappingRequest::NotFound) {
            ...
            QString errorInfo =
                "Could not map asset name: " + mapping.left(mapping.length() - QString::number(copy).length() - 1);
            qWarning(interfaceapp) << "Error downloading model: " + errorInfo;
            ...
            QString errorInfo =
                "Too many copies of asset name: " + mapping.left(mapping.length() - QString::number(copy).length() - 1);
            qWarning(interfaceapp) << "Error downloading model: " + errorInfo;
            addAssetToWorldError(filenameFromPath(filePath), errorInfo);
        }
        request->deleteLater();
...
void Application::addAssetToWorldUpload(QString filePath, QString mapping, bool isZip, bool isBlocks) {
    return;  // CPM -- don't want to suport this path at the moment.
...
void Application::addAssetToWorldAddEntity(QString filePath, QString mapping) {
    ...
    if (filePath.toLower().endsWith(PNG_EXTENSION) || filePath.toLower().endsWith(JPG_EXTENSION)) {
        ...
        properties.setModelURL("atp:" + mapping);  // CPM ADD SUPPORT FOR LOD TO THIS
        properties.setShapeType(SHAPE_TYPE_SIMPLE_COMPOUND);
    }
    properties.setCollisionless(true);  // Temporarily set so that doesn't collide with avatar.
    properties.setVisible(false);       // Temporarily set so that don't see at large unresized dimensions.

    /*  // CPM - Commented out this functionality.  It's never been a good idea or useful to creators.
    ...
    */
    ...
    glm::vec3 positionOffset =
        getMyAvatar()->getWorldOrientation() * (getMyAvatar()->getSensorToWorldScale() * glm::vec3(0.0f, 0.0f, -2.0f));
...
void Application::addAssetToWorldCheckModelSize() {
    ...
    while (item != _addAssetToWorldResizeList.end()) {
        ...
        if (dimensions != DEFAULT_DIMENSIONS) {
            ...
            auto scale = std::min(MAXIMUM_DIMENSION / dimensions.x,
                                  std::min(MAXIMUM_DIMENSION / dimensions.y, MAXIMUM_DIMENSION / dimensions.z));
            dimensions *= scale;
        ...
        if (doResize) {
            ...
            // CPM - forced grabbable to false because reasons.
            bool grabbable = false;  //(Menu::getInstance()->isOptionChecked(MenuOption::CreateEntitiesGrabbable));
            properties.setUserData(grabbable ? GRABBABLE_USER_DATA : NOT_GRABBABLE_USER_DATA);
    ...
    */
    ...
    if (!_addAssetToWorldErrorTimer.isActive()) {
        if (!_addAssetToWorldMessageBox) {
            _addAssetToWorldMessageBox = getOffscreenUI()->createMessageBox(OffscreenUi::ICON_INFORMATION, "Downloading Model",
...
void Application::loadDialog() {
    ModalDialogListener* dlg = OffscreenUi::getOpenFileNameAsync(_glWidget, tr("Open Script"), getPreviousScriptLocation(),
...
void Application::loadAvatarBrowser() const {
    ...
    QString MARKETPLACES_INJECT_SCRIPT_PATH =
        "file:///" + qApp->applicationDirPath() + "/scripts/system/html/js/marketplacesInject.js";
...
void Application::takeSnapshot(bool notify, bool includeAnimated, float aspectRatio, const QString& filename) {
    postLambdaEvent([notify, includeAnimated, aspectRatio, filename, this] {
        ...
        QString path =
            DependencyManager::get<Snapshot>()->saveSnapshot(getActiveDisplayPlugin()->getScreenshot(aspectRatio), filename,
...
void Application::takeSecondaryCameraSnapshot(const bool& notify, const QString& filename) {
    postLambdaEvent([notify, filename, this] {
        QString snapshotPath =
            DependencyManager::get<Snapshot>()->saveSnapshot(getActiveDisplayPlugin()->getSecondaryCameraScreenshot(), filename,
                                                             TestScriptingInterface::getInstance()->getTestResultsLocation());
...
void Application::takeSecondaryCamera360Snapshot(const glm::vec3& cameraPosition,
...
void Application::windowMinimizedChanged(bool minimized) {
    ...
    std::call_once(once, [&] {
        connect(&_minimizedWindowTimer, &QTimer::timeout, this, [] {
            QCoreApplication::postEvent(QCoreApplication::instance(), new QEvent(static_cast<QEvent::Type>(Idle)),
...
static void addDisplayPluginToMenu(const DisplayPluginPointer& displayPlugin, int index, bool active) {
    ...
    auto action = menu->addActionToQMenuAndActionHash(parent, name, QKeySequence(Qt::CTRL + (Qt::Key_0 + index)), qApp,
                                                      SLOT(updateDisplayMode()), QAction::NoRole, Menu::UNSPECIFIED_POSITION,
                                                      groupingMenu);
...
void Application::updateLoginDialogPosition() {
    ...
    if (pointAngle > LOOK_AWAY_THRESHOLD_ANGLE || outOfBounds) {
        ...
        {
            glm::vec3 keyboardLocalOffset =
                cameraOrientation * glm::vec3(-0.4f * getMyAvatar()->getSensorToWorldScale(), -0.3f, 0.2f);
...
void Application::showUrlHandler(const QUrl& url) {
    ...
    ModalDialogListener* dlg =
        OffscreenUi::asyncQuestion("Confirm openUrl", "Do you recognize this path or code and want to open or execute it: " +
                                                          url.toDisplayString());
    QObject::connect(dlg, &ModalDialogListener::response, this, [=](QVariant answer) {
        QObject::disconnect(dlg, &ModalDialogListener::response, this, nullptr);
