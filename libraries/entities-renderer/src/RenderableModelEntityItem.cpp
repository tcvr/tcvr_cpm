...
bool ModelEntityRenderer::needsRenderUpdate() const {
    ...
    ModelPointer model;
    QUrl parsedModelURL;
    QUrl parsedModelURL_LOD1;
    QUrl parsedModelURL_LOD2;
    QUrl parsedModelURL_LOD3;

    withReadLock([&] {

        model = _model;
        model_LOD1 = _model_LOD1;
        model_LOD2 = _model_LOD2;
        model_LOD3 = _model_LOD3;

        parsedModelURL = _parsedModelURL;
        parsedModelURL_LOD1 = _parsedModelURL_LOD1;
        parsedModelURL_LOD2 = _parsedModelURL_LOD2;
        parsedModelURL_LOD3 = _parsedModelURL_LOD3;
...
bool ModelEntityRenderer::needsRenderUpdateFromTypedEntity(const TypedEntityPointer& entity) const {
    if (resultWithReadLock<bool>([&] {
        ...
        if (_parsedModelURL_LOD1 != entity->getModelURL_LOD1()) {  // CPM
            return true;
        }

        if (_parsedModelURL_LOD2 != entity->getModelURL_LOD2()) {  // CPM
            return true;
        }

        if (_parsedModelURL_LOD3 != entity->getModelURL_LOD3()) {  // CPM
            return true;
        }
...
void ModelEntityRenderer::doRenderUpdateSynchronousTyped(const ScenePointer& scene, Transaction& transaction, const TypedEntityPointer& entity) {
    ...
    withWriteLock([&] {
        ...
        if (_parsedModelURL_LOD1 != entity->getModelURL_LOD1()) {       // CPM
            _parsedModelURL_LOD1 = QUrl(entity->getModelURL_LOD1());    // CPM
        }
        if (_parsedModelURL_LOD2 != entity->getModelURL_LOD2()) {       // CPM
            _parsedModelURL_LOD2 = QUrl(entity->getModelURL_LOD2());    // CPM
        }
        if (_parsedModelURL_LOD3 != entity->getModelURL_LOD3()) {       // CPM
            _parsedModelURL_LOD3 = QUrl(entity->getModelURL_LOD3());    // CPM
        }
    ...
    ModelPointer model;
    withReadLock([&] {         
        // here we need to add some model switching logic based on the distance bias
        if (true) model = _model;  // CPM how about we try switching between the LODs on this right here
        if (false) model = _model_LOD1; // CPM
        if (false) model = _model_LOD2; // CPM
        if (false) model = _model_LOD3; // CPM  
    });  
...
void ModelEntityRenderer::doRender(RenderArgs* args) {
    ...
    DependencyManager::get<GeometryCache>()->   (args, batch, greenColor); // CPM EMPTY MODEL GREEN CUBE
